...
| Веб-интерфейс | Браузерный интерфейс для генерации, управления адресами, ящика и партий. |
...
Веб-интерфейс даёт те же операции, что и CLI, но из браузера. Он работает
локально, вызывает те же функции, что и команды, и читает и пишет те же файлы
`cookies.txt`, `emails.txt` и `hidemyemail.db`.

```bash
uv run hidemyemail webui --open
```

Интерфейс доступен на <http://127.0.0.1:8765/> и состоит из шести панелей:

| Панель | Назначение |
| --- | --- |
| Обзор | Проверка аккаунта по cookie, вставка новой сессии iCloud, оценка остатка лимита, счётчики локальной базы. |
| Генерация | Генерация и резервирование адресов с меткой, количеством и партией. |
| Адреса iCloud | Список адресов, включение и отключение пересылки, правка метки и заметки. |
| Локальные адреса | Фильтры по базе, статусы `unused`/`used`/`trash`, синхронизация из iCloud, скачивание `addresses.csv` и `messages.csv`. |
| Ящик | Настройка IMAP, синхронизация почты, коды подтверждения, отметка о прочтении. |
| Партии | Создание, пауза, возобновление, остановка и просмотр партий, скачивание CSV с адресами партии. |

Параметры:

| Параметр | Описание |
| --- | --- |
| `--host` | Интерфейс для прослушивания. По умолчанию `127.0.0.1`. |
| `--port` | Порт. По умолчанию `8765`. |
| `--token` | Токен доступа: страница запросит его. Генерируется автоматически, если `--host` не локальный. |
| `--open` | Открыть интерфейс в браузере после запуска. |
| `--cookie-file` | Путь к файлу cookie. По умолчанию `cookies.txt`. |
| `--output` | Файл, куда дописываются адреса. По умолчанию `emails.txt`. |
| `--db-file` | Локальная база. По умолчанию `hidemyemail.db`. |
| `--config-file` | Настройки ящика. По умолчанию `inbox_config.json`. |
| `--export-dir` | Каталог для экспорта CSV. По умолчанию `exports`. |
| `--region` | `global` или `china`. |

Развёртывание в Kubernetes описано в [`deploy/k8s/README.md`](deploy/k8s/README.md);
образ контейнера — `ghcr.io/0ekk/hidemyemail-generator`.

Панель «Обзор» позволяет заменить сохранённую сессию iCloud: вставьте Header
String или весь текст `Copy as cURL` — он проверяется в iCloud до замены
текущей сессии. Отклонённая вставка ничего не меняет. Новая сессия работает
со следующего запроса, без перезапуска.

По умолчанию сервер слушает только петлевой интерфейс. Любой, кто до него
дотянется, получит контроль над аккаунтом iCloud из файла cookie, поэтому
оставляйте `127.0.0.1`, если не задаёте токен и не доверяете сети.
...
- Веб-интерфейс по умолчанию слушает `127.0.0.1`, отклоняет межсайтовые запросы на запись и требует токен при выходе за пределы петлевого интерфейса.
