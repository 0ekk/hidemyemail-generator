...
The Overview panel can replace the saved iCloud session: paste the Header
String or the whole `Copy as cURL` text, and it is validated against iCloud
before it replaces the current one. A rejected paste changes nothing. The new
session applies to the next request, with no restart.
